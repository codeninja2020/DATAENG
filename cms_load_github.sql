...
BEGIN TRY
    EXEC msdb.dbo.sp_help_job @job_name = N'CMS_S3_Import';
    SET @jobExists = 1;
END TRY
BEGIN CATCH
    SET @jobExists = 0;
...
-- 1. Make sure final table exists in a relaxed form
IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'cms')
    EXEC('CREATE SCHEMA cms AUTHORIZATION dbo');
GO

IF OBJECT_ID('cms.Dining', 'U') IS NOT NULL
    DROP TABLE cms.Dining;
GO
...
-- 6. Check final table
SELECT COUNT(*) AS FinalRowCount FROM cms.Dining;
SELECT TOP 20 * FROM cms.Dining;
