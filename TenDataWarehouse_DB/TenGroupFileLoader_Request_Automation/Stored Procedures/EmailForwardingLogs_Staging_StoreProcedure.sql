CREATE OR ALTER PROCEDURE dbo.usp_TenGroupFileLoader_LoadEmailForwardingLogs
    @LinkedServer NVARCHAR(128) = N'TENMAID_GLOBAL_PROD'
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    DECLARE @PrevVersion  BIGINT;
    DECLARE @CurrVersion  BIGINT;
    DECLARE @RowCount     INT;
    DECLARE @SQL          NVARCHAR(MAX);
    DECLARE @TableName    NVARCHAR(200) = N'EmailForwardingLogs';
    DECLARE @SchemaName   NVARCHAR(50)  = N'Request_Automation';
    DECLARE @ServerId     INT           = 2;
    
    /* 1. Read config from SSIS_TableConfig */

    SELECT @LoadType    = tc.LoadType,
           @PrevVersion = ISNULL(tc.ChangeTrackingVersion, 0)
    FROM dbo.SSIS_TableConfig tc
        INNER JOIN dbo.SSIS_ServerConfig sc ON tc.ServerId = sc.ServerId
    WHERE tc.TableName  = @TableName
      AND tc.SchemaName = @SchemaName
      AND sc.ServerId   = @ServerId;
    ...
    /* ────────────────────────────────────────────────────────────────
       2. Read Change Tracking CURRENT VERSION from source
       ──────────────────────────────────────────────────────────────── */
    SET @SQL = N'SELECT @v = ct_ver 
                 FROM OPENQUERY(' + QUOTENAME(@LinkedServer) + 
                 N', ''SELECT CHANGE_TRACKING_CURRENT_VERSION() AS ct_ver'')';

    EXEC sp_executesql @SQL, N'@v BIGINT OUTPUT', @CurrVersion OUTPUT;

    PRINT 'LoadType: ' + @LoadType
        + '  PrevVersion: ' + CAST(@PrevVersion AS VARCHAR(20))
        + '  CurrVersion: ' + CAST(@CurrVersion AS VARCHAR(20));
    ...
    BEGIN
        ...
        TRUNCATE TABLE [TenGroupFileLoader_Request_Automation].[EmailForwardingLogs];
        ...
        SET @SQL = N'
            ...
            FROM ' + QUOTENAME(@LinkedServer) + 
               N'.[TenGroupFileLoader].[request_automation].[EmailForwardingLogs]';

        EXEC sp_executesql @SQL;
        SET @RowCount = @@ROWCOUNT;

        PRINT 'Full load complete: ' + CAST(@RowCount AS VARCHAR(20)) + ' rows';
    END

    /*3B. INCREMENTAL LOAD USING CHANGE TRACKING */
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            -- Wipe staging table
            TRUNCATE TABLE [TenGroupFileLoader_Request_Automation].[EmailForwardingLogs_Staging];

            /* Apply deletes directly to target */
            SET @SQL = N'
                DELETE d
                ...
                WHERE EXISTS (
                    ...
                    FROM OPENQUERY(' + QUOTENAME(@LinkedServer) + N',
                        ''SELECT ct.LogId
                          ...
                          WHERE ct.SYS_CHANGE_OPERATION = ''''D'''''') src
